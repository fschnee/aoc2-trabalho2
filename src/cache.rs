...
impl Cache {
                  ...
                  kind: Kind) -> Cache
    {
        Cache {
            ...
            info: Info {nsets, bsize, repl, assoc, size: bsize * nsets * assoc},
            data: {
                let mut vec: Vec<Vec<Data>> = Vec::with_capacity(nsets);

                for _index_conjunto in 0..nsets {
                    let mut conjunto = Vec::with_capacity(assoc);
                    for _index_via in 0..assoc {conjunto.push(Data {tag: 0, is_initialized: false})}
                    vec.push(conjunto);
                }
